...
If you prefer not to reboot the node, you can set the variable `disable_reboot_request: true`. In that case you will need to manually restart the Docker daemon on the nodes.

## Supported parameters

- `max_size` (string) - Maximum size of a single log file before rotation. Format: `<number>(k\|m\|g)`, e.g. `100m`, `1g`.
- `max_file` (string) - Maximum number of rotated log files to retain. Must be a positive integer string, e.g. `"3"`.
